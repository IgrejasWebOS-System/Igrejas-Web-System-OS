...
// ── Mapeamento de módulos para nav ────────────────────────────────────────────
const MODULE_NAV: Record<ModuleKey, { label: string; icon: string; href: string }> = {
  membros:    { label: "Membros",      icon: "membros",    href: "/dashboard/membros" },
  financeiro: { label: "Financeiro",   icon: "financeiro", href: "/dashboard/financeiro" },
  escola:     { label: "Escola",       icon: "escola",     href: "/dashboard/escola" },
  cursos:     { label: "Cursos",       icon: "cursos",     href: "/dashboard/cursos" },
  ebd:        { label: "EBD",          icon: "ebd",        href: "/dashboard/ebd" },
  secretaria: { label: "Secretaria",   icon: "secretaria", href: "/dashboard/secretaria" },
  patrimonio: { label: "Patrimônio",   icon: "patrimonio", href: "/dashboard/patrimonio" },
  eventos:    { label: "Eventos",      icon: "eventos",    href: "/dashboard/eventos" },
  ocorrencias:{ label: "Ocorrências",  icon: "ocorrencias",href: "/dashboard/ocorrencias" },
};
  ...
  function NavItem({ icon, label, href, active }: { icon: string; label: string; href: string; active: boolean }) {
    return (
      <a
        ...
        onMouseEnter={e => {
          if (!active) (e.currentTarget as HTMLAnchorElement).style.background = "var(--color-bg)";
        }}
        onMouseLeave={e => {
    ...
    <aside style={{
      ...
      <nav style={{ flex: 1, overflowY: "auto", overflowX: "hidden", padding: collapsed ? "12px 8px" : "12px 10px" }}>
        ...
        <NavItem
          ...
          active={pathname.startsWith("/dashboard/painel")}
        />

        {/* Separador */}
        {!collapsed && (
          <p style={{ fontSize: 9, fontWeight: 800, color: "var(--color-text-muted)", textTransform: "uppercase", letterSpacing: "0.1em", padding: "12px 6px 4px", opacity: 0.7 }}>
            Módulos
          </p>
        )}
          ...
          return (
            <span key={mod}>
              <NavItem
                ...
                label={nav.label}
                href={nav.href}
                active={pathname === nav.href || (pathname.startsWith(nav.href) && !pathname.startsWith(nav.href + "/aniversariantes"))}
              />
              {/* Sub-itens secretaria */}
              {mod === "secretaria" && (
                <>
                  <NavItem icon="bolo"        label="Aniversariantes" href="/dashboard/secretaria/aniversariantes"  active={pathname.startsWith("/dashboard/secretaria/aniversariantes")} />
                  <NavItem icon="credencial"  label="Credenciais"     href="/dashboard/secretaria/credenciais"      active={pathname.startsWith("/dashboard/secretaria/credenciais")} />
                  <NavItem icon="precadastro" label="Pré-Cadastros"   href="/dashboard/secretaria/pre-cadastros"    active={pathname.startsWith("/dashboard/secretaria/pre-cadastros")} />
                  <NavItem icon="consagracao" label="Consagrações"    href="/dashboard/secretaria/consagracoes"     active={pathname.startsWith("/dashboard/secretaria/consagracoes")} />
                  <NavItem icon="batismo"     label="Batismos"        href="/dashboard/secretaria/batismos"         active={pathname.startsWith("/dashboard/secretaria/batismos")} />
                  <NavItem icon="pedidos"     label="Pedidos"         href="/dashboard/secretaria/pedidos"          active={pathname.startsWith("/dashboard/secretaria/pedidos")} />
                  <NavItem icon="pastoreio"   label="Pastoreio"       href="/dashboard/secretaria/pastoreio"        active={pathname.startsWith("/dashboard/secretaria/pastoreio")} />
                </>
              )}
            </span>
          );
        ...
        {ctx.level <= 2 && (
          <>
            ...
            {/* N0–N2 */}
            <NavItem icon="modelo"     label="Modelos de Doc."   href="/dashboard/admin/documentos/modelos"  active={pathname.startsWith("/dashboard/admin/documentos/modelos")} />
            <NavItem icon="credencial" label="Modelos Credencial" href="/dashboard/admin/credenciais/modelos" active={pathname.startsWith("/dashboard/admin/credenciais/modelos")} />
            <NavItem icon="tabelas"    label="Tabelas"            href="/dashboard/admin/tabelas/cargos"      active={pathname.startsWith("/dashboard/admin/tabelas")} />
            <NavItem icon="pedidos"    label="Catálogo de Mat."   href="/dashboard/admin/materiais"           active={pathname.startsWith("/dashboard/admin/materiais")} />
          </>
        )}
      </nav>
